...
class WxmpayService
{
    ...
    public function paycheckPaymentTo($data)
    {
        ...
        $input->SetBody($data['shop_name'] . ' ' . $data['order_name']);
        $input->SetAttach($data['goods_name']);
        $input->SetOut_trade_no($this->getNonceStr());
    ...
    public function paymentTo($data)
    {
        ...
        $input->SetAttach($goods_name_str);
        $input->SetOut_trade_no($data['order']->order_sn);
        $input->SetTotal_fee($data['order']->order_amount * 100);
        $input->SetTime_start(date("YmdHis"));
        $input->SetTime_expire(date("YmdHis", time() + 600));
        $input->SetGoods_tag($goods_name_str);
        $input->SetNotify_url($data['notify_url']);
        $input->SetTrade_type('JSAPI');
        $input->SetOpenid($openId);
        $order = WxPayApi::unifiedOrder($input);
