...
(function () {
    ...
    function setTabFocus(id) {
        ...
        switch (id) {
			...
			case 'dbgoodsimage':
				setAlign(editor.getOpt('imageManagerInsertAlign'));
				dbgoodsImage = dbgoodsImage || new DbGoodsImage('goodsImageList');
				dbgoodsImage.reset();
				break;
        }
    }
        ...
        dialog.onok = function () {
            ...
            switch (id) {
                ...
                case 'search':
                    ...
                    break;
				case 'dbgoodsimage':
					list = dbgoodsImage.getInsertList();
					break;
            }
    ...
    /* 在线商品图片 */
    function DbGoodsImage(target) {
        this.container = utils.isString(target) ? document.getElementById(target) : target;
        this.init();
    }
    DbGoodsImage.prototype = {
        init: function () {
            this.reset();
            this.initEvents();
        },
        ...
        initEvents: function () {
            ...
            /* 滚动拉取图片 */
            domUtils.on($G('goodsImageList'), 'scroll', function(e){
                var panel = this;
                if (panel.scrollHeight - (panel.offsetHeight + panel.scrollTop) < 10) {
                    _this.getImageData();
                }
        ...
        getImageData: function () {
            ...
            if(!_this.listEnd && !this.isLoadingData) {
                this.isLoadingData = true;
                var url = editor.getActionUrl(editor.getOpt('imageGoodsManagerActionName')),
                    isJsonp = utils.isCrossDomainUrl(url);
                ajax.request(url, {
                    'timeout': 100000,
                    'dataType': isJsonp ? 'jsonp':'',
